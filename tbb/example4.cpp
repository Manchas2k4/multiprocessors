/* This code will generate a fractal image. Uses OpenCV, to compile:
   g++ example4.cpp `pkg-config --cflags --libs opencv` -ltbb  */
...
private:
	...
	int juliaValue(int x, int y, const int width, const int height) const {
		int k;
...
public:
	JuliaSet(IplImage *image, int theStep) : img(image), step(theStep) {}
