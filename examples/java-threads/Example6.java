...
public class Example6 extends Thread {
	...
	public Example6(int A[], int start, int end, int numberOfThread) {
		this.A = A;
		this.start = start;
		this.end = end;
	...
	private void parallelQuick() {
		...
		if (pivot != -1) {
			...
			count++;
			lesser = new Example6(A, start, pos - 1, count);
			lesser.start();
			
			count++;
			greater = new Example6(A, pos, end, count);
			greater.start();
	...
	public static void main(String args[]) {
		...
		for (int i = 0; i < Utils.N; i++) {
			...
			obj = new Example6(Arrays.copyOf(array, array.length), 
									0, array.length - 1, 1);
			obj.start();
