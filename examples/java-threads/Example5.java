...
public class Example5 extends Thread {
	...
	public Example5(int A[], int B[], int start, int end, int numberOfThread) {
		this.A = A;
		this.B = B;
		this.start = start;
		this.end = end;
	...
	private void parallelSplit() {
		...
		count++;
		left = new Example5(A, B, start, mid, count);
		left.start();
		
		count++;
		right = new Example5(A, B, mid + 1, end, count);
		right.start();
	...
	public static void main(String args[]) {
		...
		for (int i = 0; i < Utils.N; i++) {
			...
			obj = new Example5(Arrays.copyOf(array, array.length), temp,
									0, array.length - 1, 1);
			obj.start();
