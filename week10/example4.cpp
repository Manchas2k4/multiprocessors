/* This code will generat
e a fractal image. Uses OpenCV, to compile:
   g++ example4.cpp `pkg-config --cflags --libs opencv`  */
...
private:
	...
	int juliaValue(int x, int y, int width, int height) const {
		int k;
...
public:
	JuliaSet(IplImage *image, int ssize, int sstep) 
		: img(image), size(ssize), step(sstep) {}
